...
def basic_decrypt_unscramble(im_arr, x_size, y_size, n, d):
	for x in range(x_size):
		if x % 100 == 0:
			print("Decrypting: ", x, "/", x_size)
		for y in range(y_size):
			pixel = im_arr[x, y]
			im_arr[x, y] = tuple(int(c**d % n) for c in pixel)
...
def main():
	...
	if selection == 1:
		...
		im.thumbnail((430, 430)) if im.size[0] > 430 or im.size[1] > 430 else None
